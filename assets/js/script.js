class Propietario{
    constructor(nombrePropietario, direccion, telefono){
        //como no se exigen getters y setters para esta clase
        //asignamos directamente las propiedades
...
class Animal extends Propietario{
    constructor(nombrePropietario, direccion, telefono, tipo){
        ...
        //acá si usamos getter asi que creamos una variable local
        this._tipo = tipo;
    }
...
class Mascota extends Animal{
    ...
    //getters y setters para los atributos propios de Mascota
    get nombre(){
...
form.addEventListener("submit", (e)=>{
    e.preventDefault();
    //pone los elementos input (todos de la clase form-control) en un arreglo
    let arregloInputs = Array.from(document.getElementsByClassName("form-control"))
    //convierte el arreglo de elementosHTML en un arreglo de sus valores
    arregloInputs = arregloInputs.map(input => input.value)
    //destructuramos
    let [nombrePropietario, telefono, direccion, nombreMascota, tipo, enfermedad] = arregloInputs
    //crear objeto con el nombre de la instancia según el tipo de animal
    //podría hacerse con eval pero su uso no me parece una buena práctica
    
    if (tipo == "perro"){
        const perro = new Mascota(nombrePropietario,direccion,telefono,tipo,nombreMascota,enfermedad)
        let mensajeHTML = crearMensaje(perro)
        console.log(mensajeHTML)
        ulresultado.innerHTML = mensajeHTML
    }
    else if(tipo =="gato"){
        const gato = new Mascota(nombrePropietario,direccion,telefono,tipo,nombreMascota,enfermedad)
        let mensajeHTML = crearMensaje(gato)
        ulresultado.innerHTML = mensajeHTML
    }
    else if(tipo =="conejo"){
        const conejo = new Mascota(nombrePropietario,direccion,telefono,tipo,nombreMascota,enfermedad)
